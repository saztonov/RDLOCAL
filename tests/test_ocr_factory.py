...
from rd_core.ocr import create_ocr_engine, create_async_ocr_engine
...
class TestCreateAsyncOCREngine:
    def test_async_dummy_backend(self):
        engine = create_async_ocr_engine("dummy")
        assert engine is not None
        assert hasattr(engine, "recognize_async")

    def test_async_openrouter(self):
        engine = create_async_ocr_engine("openrouter", api_key="test-key", model_name="test/model")
        assert engine is not None
        assert hasattr(engine, "recognize_async")

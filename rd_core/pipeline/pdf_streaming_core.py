...
class StreamingPDFProcessor:
    ...
    def __init__(self, pdf_path: str, zoom: float = None, *, dpi: int = DEFAULT_PDF_RENDER_DPI):
...
def get_page_dimensions_streaming(pdf_path: str, *, dpi: int = DEFAULT_PDF_RENDER_DPI) -> Dict[int, Tuple[int, int]]:
    """Получить размеры всех страниц без полного рендеринга"""
    dims = {}
    with StreamingPDFProcessor(pdf_path, dpi=dpi) as processor:
